import React, {  useContext, Fragment, useState, useLayoutEffect, useRef } from 'react';
...
const CanvasSelectElement = () => {
    ...
    const handleSelectBoxType = (e) => {
        const {clientX, clientY} = e
        const {startX, startY, startClientX , startClientY} = client
        const moveX = startX - clientX
        const moveY = startY - clientY
        const currentComponent = global.getSelectComponent()
        const {style: {width: elWidth, height: elHeight, left, top, transform}} = global.getSelectComponent()
        ...
        switch(selectBoxType) {
            ...
            case 'rotate': {
                ...
                if (!transform || transform.includes('rotate')) {
                    const reg = /\((.*?)\)/g
                    if (rotateStep === 0) {
                        rotate = parseInt(transform.match(reg)[0].replace(/\(|\)/g, '')) % 360
                        rotateStep = rotate
                    } else {
                        rotate = rotateStep
                    }
    ...
    const renderRotateLine = () => {
        const {width: elWidth, height: elHeight, left: elLeft, top: elTop} = selectComponent.style
        const width = parseInt(elWidth)
        const height = parseInt(elHeight)
        ...
        return (
            <Fragment>
                {
                ReactDOM.createPortal(<ul className='rotate-line' ref={rotateRef} style={{left: rotateLineLeft + 'px', top: rotateLineRight + 'px'}}>
                    {
                        [...new Array(4)].map((item, index) => (
                            <li className={`rotate-line-item rotate-${index + 1}`} key={index}>{item}</li>
                        ))
                    }
                </ul>, document.querySelector('.h5-canvas-draw'))
                }
            </Fragment>
        )
    }
    ...
    return <section className="canvas-select-element" style={{...selectComponent.style}} >
            {/* 旋转 */}
            <div className="icon-rotate canvas-select-rotate" data-select-box={'rotate'}></div>
            {/* 旋转参考线 */}
            {
                renderRotateLine()
            }
